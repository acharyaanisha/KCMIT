...
class ThreadCommentView extends StatefulWidget {
  final String threaduuid;
  const ThreadCommentView({super.key, required this.threaduuid});

  @override
  State<ThreadCommentView> createState() => _ThreadCommentViewState();
}

class _ThreadCommentViewState extends State<ThreadCommentView> {
  ...
  Future<void> viewComment() async {
    ...
    final url = Config.getThreadCommentView();
    try {
      final response = await http.post(
        Uri.parse(url),
        headers: {
